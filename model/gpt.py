...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.model = nn.Sequential(
            *[self.TransformerBlock(model_dim, num_heads)  for i in range(num_blocks)]
        )
        self.layernorm = nn.LayerNorm(model_dim)
        self.linearlayer = nn.Linear(model_dim, vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        T = context.shape[1]
        tok_emb = self.token_embedding(context)  # Shape: (B, T, C)
        pos = torch.arange(0, T)  # Positions 0 to T-1
        pos_emb = self.pos_embedding(pos)  # Shape: (T, C)
        x = tok_emb + pos_emb  # Broad-casted sum: (B, T, C)
